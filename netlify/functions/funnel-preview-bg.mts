...
function buildGenerationPrompt(ageBracket: string, analysis: string): string {
  ...
  return `Create a realistic "after" transformation photo of this exact person. Show optimistic but realistic 12-week progress.

${ageContextLine(age)}
${conservative ? "IMPORTANT: Keep changes conservative and realistic. No dramatic muscle gains." : ""}
...
— Show the natural V-taper: broader shoulders, narrower waist — within 12-week limits.
...
— All changes must be within physiological limits through natural training.`;
}
