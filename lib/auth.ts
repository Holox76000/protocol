...
// ──────────────────────────────────────────────
// Unsubscribe tokens live in a `server-only`-free module (lib/unsubscribeToken)
// so the email path can be bundled into standalone Netlify Functions. Re-exported
// here so existing importers of lib/auth keep working unchanged.
export { createUnsubscribeToken, verifyUnsubscribeToken } from "./unsubscribeToken";
