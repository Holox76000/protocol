...
function MHeroV1({ href, eyebrow, attractsMen, personalization }: { href: string; eyebrow: string | null; attractsMen: boolean; personalization: HeroPersonalization | null }) {
  const hasPerso = Boolean(personalization?.lp_hero_h1_em && personalization?.lp_hero_desc);

  const desc = hasPerso
    ? personalization!.lp_hero_desc!
    ...
    <section className="mo-hero mo-hero-v1">
      ...
      <div className="mo-hero-v1__left">
        <div className="mo-hero-v1__copy">
          ...
          <h1 className="mo-hero__title">{renderTitleContent()}</h1>
...
const OUTCOMES = [
  ...
  { rank: "03", category: "Dating",  label: "Matches on dating apps", delta: 81, scaleMax: 100, delay: 0.35 },
];
...
export default function F1OfferPage() {
  ...
  const [personalization, setPersonalization] = useState<HeroPersonalization | null>(null);
  ...
  useEffect(() => {
    ...
    if (sid) {
      // Fetch cached LLM personalization (read-only; generated by the report route)
      fetch(`/api/funnel/personalization?sid=${encodeURIComponent(sid)}`)
        .then((r) => r.ok ? r.json() : { personalization: null })
        .then((data) => {
          if (data?.personalization) setPersonalization(data.personalization as HeroPersonalization);
        })
        .catch(() => {});
  ...
  return (
    <div className="mo-page">
      ...
      <MHeroV1 href={signupHref} eyebrow={heroEyebrow} attractsMen={attractsMen} personalization={personalization} />
      <MPress />
      <MAdvisors />
