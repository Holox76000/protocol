...
function getAgeInsight(ageBracket: string): string {
  const map: Record<string, [string, string]> = {
    "20–29": ["Your 20s: the best window you'll have", "The gap between where you are and your peak is typically largest right now. Testosterone is near its highest point — around 25 — and your body adapts to structured training faster than it will at any later stage. Men who act in this window lock in structural changes that hold for 20+ years. The window is wide, but it narrows every year."],
    "30–39": ["Your 30s: the window is still open", "Testosterone is still near peak but dropping about 1–2% per year. Metabolism has slowed slightly, which actually makes body composition more predictable when the plan is precise. Men who make structural changes in their 30s tend to hold them longer than men who start earlier but follow generic programs. Motivation, resources, and hormonal environment rarely align this well again."],
    "40–49": ["Your 40s: random training stops working here", "High-volume, unfocused training at this stage often increases cortisol without producing visible adaptation. Men on targeted plans in their 40s see better proportional results than men half their age following generic programs. Precision matters more than effort now."],
    "50+": ["After 50: the approach changes, the potential doesn't", "Men who look worse at 55 than at 45 almost always followed the wrong program, not an inevitable biology. What changes after 50 is the work: less about adding mass, more about proportions, posture, and metabolic precision. Men on structured plans at this stage often look better than they did at 40 on generic advice."],
  };
...
function getEthnicityInsight(ethnicity: string): string {
  const map: Record<string, [string, string]> = {
    "Caucasian": ["Your build: wide frame, midsection tendency", "European-origin frames often carry natural width in the shoulders, but tend to store fat in the abdominal region first. That fat directly compresses your shoulder-to-waist ratio. The plan prioritizes waist reduction alongside upper body work to widen that gap."],
    "Black": ["Your build: high natural muscle density", "West African-derived profiles tend toward higher natural muscle density. The focus here is refining what's already there — definition and proportion over raw size. Watch out for over-developing non-key muscle groups; it can reduce visual symmetry rather than improve it."],
    "Asian (East / SE)": ["Your build: narrow frame, fast-moving proportions", "East and Southeast Asian frames typically feature slimmer bone structure and lower body fat. The main variable is shoulder width relative to your naturally narrow waist. Small gains in shoulder development create a large visible change on this build — faster than most profiles."],
    "South Asian": ["Your build: visceral fat tendency, strong recomposition response", "South Asian profiles often show higher visceral fat despite moderate total weight — linked to hormonal and metabolic factors. The waist-to-shoulder ratio is what drives the score here. Targeted abdominal work combined with upper body development produces faster visible results on this build than generic programs."],
    "Hispanic-Latino": ["Your build: responds well to muscle work, lower-abdominal tendency", "Hispanic-Latino frames tend to carry muscle well and adapt quickly to hypertrophy training. The main challenge is typically fat distribution in the lower abdominal region. Sequenced fat reduction followed by targeted structural building works well here — results come faster than most profiles."],
    "MENA": ["Your build: high development potential, recomposition-focused", "Middle Eastern and North African profiles often combine strong muscle development potential with elevated abdominal fat storage. The approach here is recomposition: build muscle density while reducing fat in the areas that most affect the waist-to-shoulder ratio. Your build responds well to this kind of structured work."],
  };
